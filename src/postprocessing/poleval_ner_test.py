...
def computeScores(model, goldfile, userfile, htype="split"):
    ...
    for nr in range(len(goldjson['questions'])):
        idGold = '/'.join(goldjson['questions'][nr]['input']['fname'].split('/'))
        # print(idGold)
            ...
            if len(goldjson['questions'][nr]['answers']) > 1:
                ...
                for i, value in enumerate(goldjson['questions'][nr]['answers']):
                    ...
                    if value > maximum:
                        maximum = value
                        index = i
                idsToAnnsGold[idGold] = goldjson['questions'][nr]['answers'][index]
            else:
                idsToAnnsGold[idGold] = goldjson['questions'][nr]['answers'][0]
    ...
    prec_overlap = float(global_tp_ov) / float(global_fp_ov + global_tp_ov)
    recall_overlap = float(global_tp_ov) / float(global_fn_ov + global_tp_ov)
    f1_overlap = float(2 * prec_overlap * recall_overlap) / float(prec_overlap + recall_overlap)
    print("MODEL: %s" % model)
    print("OVERLAP precision: %0.3f recall: %0.3f F1: %0.3f " %( prec_overlap, recall_overlap, f1_overlap))
    ...
    print("FINAL SCORE precision: %0.3f recall: %0.3f F1: %0.3f " %
          (0.8*prec_overlap + 0.2*prec, 0.8*recall_overlap + 0.2*recall, 0.8*f1_overlap + 0.2*f1))
