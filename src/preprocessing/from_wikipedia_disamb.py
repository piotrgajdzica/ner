...
def get_krnnt_tokens(sentences):
    content = requests.post('http://192.168.99.100:9003', data=' '.join(sentences).encode('utf-8')).content.decode('utf-8')
    token = None
    for line in content.split('\n'):
        if line.startswith('\t'):
            _, lemma, morph, _ = line.split('\t')
            yield token, lemma, morph
        elif line == "":
...
def tagged_sentence(sentence):
    for token, lemma, morph in get_krnnt_tokens([sentence]):
        yield token, lemma, morph
...
def joined_tokens(tagged_tokens, annotated_tokens):
    ...
    token_batch = []
    for token, lemma, morph in tagged_tokens:
        if token is None:
            token_batch.append((None, None, None, None, None))
            yield token_batch
            token_batch = []
        else:
            ...
            token_batch.append((token, lemma, morph, page_id, begin))
    if token_batch:
        yield token_batch
...
def should_skip(token_batch) -> bool:
    ...
    for token, lemma, morph, page_id, begin in token_batch[1:]:
        if token is None:
            pass
...
if __name__ == '__main__':
    ...
    print(last_counter)
    f_counter.close()
    for sentence in sentence_generator([os.path.join(input_dir, file) for file in os.listdir(input_dir)]):
        try:
            counter += 1
            ...
            for token, page_id, begin in token_generator(sentence):
                # print(token, page_id, begin)
                annotated_tokens.append((token, page_id, begin))
            for token, lemma, morph in tagged_sentence(clear_sentence(sentence)):
                tagged_tokens.append((token, lemma, morph))
            ...
            for token_batch in joined_tokens(tagged_tokens, annotated_tokens):
                if not should_skip(token_batch):
                    for token, lemma, morph, page_id, begin in token_batch:
                        if token is None:
                            f.write('\n')
                        else:
                            ...
                            f.write('%s\t%s\t%s\t%s\t%s_%s\t%s_%s\n' % (
                                token, lemma, '_', morph, begin or '', nkjp_class or '',
